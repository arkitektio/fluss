"""fluss must stay importable, and correct, without arkitekt.

arkitekt is an optional integration. Two places may know about it: the integration
module ``fluss/arkitekt.py``, and ``fluss/__init__.py``, which imports that module
behind ``try/except ImportError``. Everything else, and above all the code that
decides which client answers a call, takes references instead of importing.
"""
...
PACKAGE = Path(fluss.__file__).parent
...
def test_only_the_integration_module_imports_arkitekt() -> None:
    ...
    assert not offenders, (
        f"{offenders} import arkitekt at module scope. fluss works without arkitekt: "
        "take a reference (`ctx=`, `rath=`) instead of importing it."
    )


def test_fluss_imports_with_arkitekt_unavailable() -> None:
    ...
    script = (
        ...
        "assert hasattr(Fluss, 'aget_flow')\n"
        "print('ok')\n"
    )
