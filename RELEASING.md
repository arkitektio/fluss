...
`jhnnsrs/fluss:${FLUSS_SERVICE_TAG:-latest}`, and `integration.yaml` sets the same
default. Every branch is therefore tested against `:latest`, the released line.

To test against another published tag, set the repository variable `FLUSS_SERVICE_TAG`
